...
function update()
  ...
  if autoOutputGate == 1 and ri.status == "running" then 
    ...
    local normalisationFactor =  differenceInSaturation / (ri.maxEnergySaturation - desiredSaturation) 
    ...
    if satPercent > targetSatPercent then
      coarseAdjustments = scale * normalisationFactor * math.exp(-scale) + 1
      desiredFlow = desiredGeneration * coarseAdjustments
      --print("adjustment1: " ..  coarseAdjustments)
    else
      coarseAdjustments = - (scale * normalisationFactor * math.exp(-normalisationFactor)) + 1
      desiredFlow = desiredGeneration * coarseAdjustments
